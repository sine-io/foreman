...
import (
	...
	appquery "github.com/sine-io/foreman/internal/app/query"
...
func TestRunWorkbenchKeepsOlderRunPrimarySummaryWhenNewerRetryExists(t *testing.T) {
	db := OpenTestDB(t)
	taskID := seedTaskGraph(t, db)
	repo := NewBoardQueryRepository(db)
	...
	mustExec(
		...
		db,
		`update tasks set summary = ?, state = ? where id = ?`,
		"Inspect multi-run run workbench summary selection",
		"failed",
		taskID,
	)
	saveRunRow(t, db, ports.Run{
	...
	mustExec(
		t,
		db,
		`insert into artifacts (id, task_id, run_id, kind, path, summary, created_at) values (?, ?, ?, ?, ?, ?, ?)`,
		"artifact-after-new-run",
		taskID,
		"run-new",
		"assistant_summary",
	...
	mustExec(
		...
		db,
		`insert into artifacts (id, task_id, run_id, kind, path, summary, created_at) values (?, ?, ?, ?, ?, ?, ?)`,
		"artifact-for-old-run",
		taskID,
		"run-old",
		"assistant_summary",
		"artifacts/tasks/task-1/assistant-old.txt",
		"Older run failed before retry",
		"2026-03-29T10:00:00.000000000Z",
	)

	view, err := appquery.NewRunWorkbenchQuery(repo).Execute("run-old")
	require.NoError(t, err)
	require.Equal(t, "run-old", view.RunID)
	require.Equal(t, "Older run failed before retry", view.PrimarySummary)
	require.Len(t, view.Artifacts, 2)
	require.Equal(t, []string{"artifact-after-new-run", "artifact-for-old-run"}, []string{view.Artifacts[0].ID, view.Artifacts[1].ID})
}
